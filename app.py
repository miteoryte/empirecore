...
def _fetch_article_text(url: str) -> str:
    """Попытаться извлечь основной текст статьи по URL."""
    import requests as req
...
def news_action(nid):
    ...
    if action == 'publish':
        try:
            with db() as conn:
                cur = conn.cursor()
                # Получаем данные новости + привязанный telegram-канал через страну источника
                cur.execute("""
                    SELECT n.*,
                           t.bot_token, t.chat_id,
                           n.generated_title  AS g_title,
                           n.original_title   AS o_title,
                           n.generated_text   AS g_text,
                           n.original_url     AS url,
                           n.image_url        AS img
                    FROM news n
                    LEFT JOIN sources s ON n.source_id = s.id
                    LEFT JOIN telegram_channels t ON t.country_id = s.country_id AND t.status='connected'
                    WHERE n.id = %s
                    LIMIT 1
                """, (nid,))
                row = cur.fetchone()
            ...
            title     = row['g_title'] or row['o_title'] or ''
            text      = row['g_text'] or ''
            image_url = row['img'] or ''
            bot_token = row['bot_token']
            chat_id   = row['chat_id']

            # Если текст пустой — пробуем подтянуть с сайта
            if not text and row['url']:
                text = _fetch_article_text(row['url'])

            tg_result = {'ok': False, 'description': 'Нет привязанного Telegram-канала'}
            if bot_token and chat_id:
                tg_result = _send_to_telegram(bot_token, chat_id, title, text, image_url)

            # Обновляем статус в любом случае
            with db() as conn:
                cur = conn.cursor()
                cur.execute("UPDATE news SET status='published' WHERE id=%s", (nid,))
            ...
            if tg_result.get('ok'):
                ...
                return jsonify({
                    ...
                    'telegram':'failed',
                    'telegram_error': tg_result.get('description','')
                })
...
@login_required
def instant_parse():
    """Запустить парсинг всех включённых источников прямо сейчас."""
    try:
        with db() as conn:
            cur = conn.cursor()
            ...
            for item in result.get('items', []):
                try:
                    with db() as conn:
                        ...
                        cur.execute("SELECT id FROM news WHERE original_url=%s", (item['link'],))
                        if cur.fetchone():
                            continue
                        cur.execute("""
                            INSERT INTO news (source_id, original_title, original_url, image_url, status, parsed_at)
                            VALUES (%s, %s, %s, %s, 'new', NOW())
                        """, (src['id'], item['title'], item['link'], item.get('image','')))
                        saved += 1
                except Exception:
                    pass

            # Обновляем last_parsed_at
            try:
                with db() as conn:
                    cur = conn.cursor()
                    cur.execute("UPDATE sources SET last_parsed_at=NOW() WHERE id=%s", (src['id'],))
            except Exception:
                pass
    ...
    msg = f"Сохранено {saved} новых материалов"
    if errors:
        msg += f". Ошибки: {'; '.join(errors[:3])}"

    return jsonify({'status':'ok','saved':saved,'message':msg})
